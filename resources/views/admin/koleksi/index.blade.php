<x-dashboard-layout title="Manajemen Koleksi">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.koleksi.edit', $koleksi) }}" 
                                         ...
                                         <form id="delete-form-{{ $koleksi->id }}" action="{{ route('admin.koleksi.destroy', $koleksi) }}" method="POST" style="display:none;">
                                             @csrf
                                             @method('DELETE')
                                         </form>
                                     </div>
    ...
    <div id="modal-confirm-delete" style="display:none; position:fixed; inset:0; z-index:9999; background:rgba(0,0,0,0.5); align-items:center; justify-content:center;">
        <div style="background:#fff; border-radius:20px; padding:28px 32px; max-width:420px; width:90%; box-shadow:0 25px 50px rgba(0,0,0,0.15);">
            <div style="display:flex; align-items:center; gap:14px; margin-bottom:16px;">
                ...
                <div>
                    <p style="font-size:16px;font-weight:700;color:#18181b;margin:0;">Hapus Koleksi</p>
                    <p style="font-size:13px;color:#71717a;margin:4px 0 0 0;" id="confirm-delete-name">Apakah Anda yakin?</p>
                </div>
            </div>
            <p style="font-size:13px;color:#52525b;margin-bottom:22px;">Tindakan ini tidak dapat dibatalkan. Data koleksi ini akan dihapus secara permanen.</p>
    ...
    function confirmDelete(id, title) {
        ...
        if (el) el.textContent = 'Hapus koleksi: ' + title;
